...
class HeadersGenerator:    
    ...
    def ros_md5sum(self, srv):
        ...
        for l in text.split('\n'):
            ...
            if "geometry_msgs/" in l or "std_msgs/" in l or "sensor_msgs/" in l or "Header" in l: # need to replace it by corresponfing md5
              message = l.split(' ')[0]
              if message in self.predifinedMD5:
                  md5 = self.predifinedMD5[message]
            ...
            if l.startswith('---'): #lenient, by request
                ...
                accum.write(l+'\n')
        m.update(text_in.getvalue().strip())
        m.update(text_out.getvalue().strip())
        return m.hexdigest()
